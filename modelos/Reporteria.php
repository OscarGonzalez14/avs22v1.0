...
public function facturaManualData($correlativo,$cliente){
    $conectar= parent::conexion();
	parent::set_names();

	$sql= "select df.correlativo,df.cantidad,df.descripcion,f.n_correlativo,f.titular,f.monto,df.p_unitario,df.subtotal,f.retencion from correlativo_factura as f INNER join det_factura_manual as df on df.correlativo=f.n_correlativo where df.correlativo=? and f.n_correlativo=? and f.titular=? order by df.id_det_fm ASC;";
		$sql=$conectar->prepare($sql);
