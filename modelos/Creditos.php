...
/////////////////////GET CORRELATIVO FACTURAS
public function getCorrelativoCcf($sucursal){
  $conectar= parent::conexion();
  $suc='%'.$sucursal.'%';
  $sql= "select n_correlativo+1 as n_correlativo from comprobantes_ccf where sucursal like ? order by id_comprobante desc limit 1;";
  $sql=$conectar->prepare($sql);
  $sql->bindValue(1,$suc);
  $sql->execute();
  return $resultado= $sql->fetchAll(PDO::FETCH_ASSOC);
}
public function validaCcf($correlativo){
    $conectar= parent::conexion();
    $sql = "select n_correlativo from comprobantes_ccf where n_correlativo=?;";
    $sql=$conectar->prepare($sql);
...
public function registrarCcf($correlativo,$sucursal,$usuario,$cliente,$n_registro,$nit,$giro,$monto,$n_venta){

    $conectar= parent::conexion();
    $hoy = date("d-m-Y H:i:s");
    $estado = 'Cancelado';

    $sql = "insert into comprobantes_ccf values(null,?,?,?,?,?,?,?,?,?,?,?);";
    $sql=$conectar->prepare($sql);
    $sql->bindValue(1,$correlativo);
    ...
    $sql->bindValue(5,$hoy);
    $sql->bindValue(6,$cliente);
    $sql->bindValue(7,$monto);
    $sql->bindValue(8,$n_registro);
    $sql->bindValue(9,$nit);
    $sql->bindValue(10,$giro);
    $sql->bindValue(11,$estado);
    $sql->execute();

}

public function validaCorrelativoFactManual($correlativo){
    $conectar= parent::conexion();
    parent::set_names();
    $conectar= parent::conexion();
    $sql = "select n_correlativo from correlativo_factura where n_correlativo=?;";
    $sql=$conectar->prepare($sql);
    $sql->bindValue(1,$correlativo);
    $sql->execute();
    return $resultado= $sql->fetchAll(PDO::FETCH_ASSOC);
...
public function registrarFacManual(){
    $conectar = parent::conexion();
    parent::set_names();

    $detalles_fm = array();
    $detalles = json_decode($_POST["arrayDetFactMan"]);
    ...
    foreach ($detalles as $key => $v) {
        $cant_fac_man = $v->cant_fac_man;
        $desc_fac_man = $v->desc_fac_man;
        $precio_fac_man = $v->precio_fac_man;
        $subt_fac_man = $v->subt_fac_man;

        $dets = "insert into det_factura_manual values(null,?,?,?,?,?,?,?,?);";
        $dets=$conectar->prepare($dets);          
        $dets->bindValue(1,$correlativo);
        $dets->bindValue(2,$cant_fac_man);
        $dets->bindValue(3,$desc_fac_man);
        $dets->bindValue(4,$precio_fac_man);
        $dets->bindValue(5,$subt_fac_man);
        $dets->bindValue(6,$hoy);
        $dets->bindValue(7,$id_usuario);
        $dets->bindValue(8,$sucursal);
        $dets->execute();
    }
