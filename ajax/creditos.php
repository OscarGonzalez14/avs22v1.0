...
switch ($_GET["op"]){
  ...
  foreach($datos as $row){
    ...
    $sub_array[] = '<button type="button"  class="btn '.$atrib.' btn-sm" onClick="'.$event_ccf.'('.$row["id_paciente"].',\''.$row["numero_venta"].'\',\''.$row["nombres"].'\',\''.number_format((float)$row["monto"],2,".",",").'\');" ><i class="'.$icon.'"></i>'.$txt.'</button>'; 
    $data[] = $sub_array;
  }
...
/************************************************************
  ...
  case 'get_correlativo_ccf':
    ...
    $datos=$creditos->getCorrelativoCcf($sucursal);
    ...
    case 'registrar_correlativo_ccf':
    $correlativo = $creditos->validaCcf($_POST["correlativo"]);
      if (is_array($correlativo)==true and count($correlativo)==0) {
            $creditos->registrarCcf($_POST['correlativo'],$_POST['sucursal'],$_POST['usuario'],$_POST['cliente'],$_POST['n_registro'],$_POST['nit'],$_POST['giro'],$_POST['monto'],$_POST['n_venta']);
        $messages = "ok";
      }else{
        $messages = "error";
      }
    ...
    case 'registrar_factura_manual':
      $correlativo = $creditos->validaCorrelativoFactManual($_POST["correlativo"]);
      if (is_array($correlativo)==true and count($correlativo)==0) {
        $creditos->registrarFacManual();
        $mensaje = "Ok";
      }else{
        $mensaje = "Error";
      }
